...
#
#  @param model_file The output file generated by cmapss-experiment.py
#
#  @param dataset_num CMAPSS dataset number: 1, 2, 3 or 4
#
#  @param D The autoregressive order (must correspond to that of the given model)
#
#  @param begin_index, nb_seq Allow to define the starting and the ending 
#   points of training, validation and test datasets.
#   * Training datasets = the 60% first time series 
#   * Validation datasets = the next 20% time series from the end of taining datasets
#   * Test datasets = the last 20% time series
#  E.g., for dataset_num=1 composed of 100 time series, we get: 
#  (begin_index=0, nb_seq=60), (begin_index=60, nb_seq=20) and 
#  (begin_index=80, nb_seq=100) for training, validation and test datasets, 
#  respectively. 
#



## @fn cmapss_inference_task
#  @brief Perform inference on CMAPSS time series
#
...
#
def confusion(model_file, dataset_num, begin_index, nb_seq, D):
    X_order = D
    
    #---data loading
    data_file = "data/cmapss-data/train_FD00" + str(dataset_num) + "_LHI_state"
    infile = open(data_file,'rb')
    data_set = pickle.load(infile)
    infile.close()
